...
from agents.pdqn_utils import (soft_update_target_network, hard_update_target_network,
                               OrnsteinUhlenbeckActionNoise, Memory, Agent)
...
class QActor(nn.Module):
    ...
    def __init__(self, state_size, action_size, action_parameter_size):
        ...
        self.layers_1.append(nn.Linear(inputSize, 256))
        self.layers_1.append(nn.Linear(256, 256))
        self.layers_1.append(nn.Linear(256, self.action_size))

        self.layers_2 = nn.ModuleList()
        inputSize = self.state_size + self.action_parameter_size
        self.layers_2.append(nn.Linear(inputSize, 256))
        self.layers_2.append(nn.Linear(256, 256))
        self.layers_2.append(nn.Linear(256, self.action_size))
...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        self.actor = actor_class(self.observation_space[0][0], self.num_actions, self.action_parameter_size).to(device)
        self.actor_target = actor_class(self.observation_space[0][0], self.num_actions, self.action_parameter_size).to(device)
        hard_update_target_network(self.actor, self.actor_target)
        self.actor_target.eval()

        self.actor_param = actor_param_class(self.observation_space[0][0], self.num_actions, self.action_parameter_size).to(device)
        self.actor_param_target = actor_param_class(self.observation_space[0][0], self.num_actions, self.action_parameter_size).to(device)
    ...
    def act(self, state):
        with torch.no_grad():
            ...
            if self.use_ornstein_noise and self.noise is not None:
                all_action_parameters[0:self.action_parameter_size] += self.noise.sample()[0:self.action_parameter_size]
