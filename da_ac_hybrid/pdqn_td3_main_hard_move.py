"""
This file is based on the HyAR implementation at:
https://github.com/TJU-DRL-LAB/self-supervised-rl/blob/ece95621b8c49f154f96cf7d395b95362a3b3d4e/RL_with_Action_Representation/HyAR/Raw_RL/main_hard_move_pdqn_td3.py

Reference commit: ece95621b8c49f154f96cf7d395b95362a3b3d4e
"""
import argparse
import os
import time
...
from agents.pdqn_utils import OrnsteinUhlenbeckActionNoise
...
def evaluate(env, agent, max_steps, action_n_dim, epsilon, episodes=1000):
    returns = []
    epioside_steps = []
    success = []
    agent.epsilon = 0.
    ...
    agent.noise = OrnsteinUhlenbeckActionNoise(2 ** action_n_dim, mu=0.,
...
def run(args):
    ...
    dir = "result/PDQN/simple_move_4_direction_v1_"+"{}".format(args.action_n_dim)
    data = "log"
    redir = os.path.join(dir, data)
    if not os.path.exists(redir):
        os.makedirs(redir)
    print("redir", redir)
    # title1 = "Reward_pdqn_simple_move_4_direction_v1_4_"
    title2 = "Test_Reward_100_pdqn_td3_simple_move_4_direction_v1_"
    title3 = "Test_success_pdqn_td3_simple_move_4_direction_v1_"
    title4 = "Test_epioside_step_pdqn_td3_simple_move_4_direction_v1_"

    # np.savetxt(os.path.join(redir, title1 + "{}".format(str(seed) + ".csv")), Reward, delimiter=',')
    ...
    np.savetxt(os.path.join(redir, title4 + "{}".format(str(args.seed) + ".csv")), Test_epioside_step_100, delimiter=',')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scenario', default='simple_move_4_direction_v1', help='name of the scenario script.', type=str)
